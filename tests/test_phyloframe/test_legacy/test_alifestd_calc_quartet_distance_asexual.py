...
from phyloframe.legacy import (
    alifestd_calc_quartet_distance_asexual as alifestd_calc_quartet_distance_asexual_,
)
from phyloframe.legacy import (
...
alifestd_calc_quartet_distance_asexual = enforce_dtype_stability_pandas(
    alifestd_calc_quartet_distance_asexual_
)
...
def test_polytomy_identical(df: pd.DataFrame):
    ...
    for adf, bdf in (df, df), (df, df.sample(frac=1)):
        assert alifestd_calc_quartet_distance_asexual(adf, bdf) == 0
...
def test_differing_wrong1():
    ...
    assert 0 < alifestd_calc_quartet_distance_asexual(adf, bdf) < 1
    assert (
        0 < alifestd_calc_quartet_distance_asexual(adf, bdf.sample(frac=1)) < 1
    )
...
def test_differing_wrong_big():
    ...
    est = alifestd_calc_quartet_distance_asexual(adf, bdf, "taxon_label")
    assert 0 < est < 1

    est = alifestd_calc_quartet_distance_asexual(
        adf,
        bdf.sample(frac=1),
        "taxon_label",
    )
...
def test_identical_polytomy():
    ...
    assert alifestd_calc_quartet_distance_asexual(adf, bdf, "taxon_label") == 0
    assert (
...
def test_differing_polytomy_asymmetrical():
    adf = pd.DataFrame(
        {
            "id": [0, 1, 2, 3, 4, 5, 6, 7, 8],
            "taxon_label": ["0", "1", "2", "3", "4", "5", "6", "7", "8"],
    ...
    assert (
        0 < alifestd_calc_quartet_distance_asexual(adf, bdf, "taxon_label") < 1
    )
    assert (
        0
        < alifestd_calc_quartet_distance_asexual(
