...
def test_alifestd_mark_ancestor_origin_time_polars_simple_tree(
    ...
    # root's ancestor is itself, so origin_time[0] = 0.0
    # node 1's ancestor is 0, so origin_time[0] = 0.0
    # node 2's ancestor is 0, so origin_time[0] = 0.0
    # node 3's ancestor is 1, so origin_time[1] = 1.0
    # node 4's ancestor is 1, so origin_time[1] = 1.0
    assert result["ancestor_origin_time"].to_list() == [
        0.0,
        0.0,
        0.0,
        1.0,
...
def test_alifestd_mark_ancestor_origin_time_polars_single_node(
    apply: typing.Callable,
):
    """A single root node's ancestor_origin_time is its own origin_time."""
    df_pl = apply(
        pl.DataFrame(
            {
                "id": [0],
    ...
    assert result["ancestor_origin_time"].to_list() == [5.0]
...
def test_alifestd_mark_ancestor_origin_time_polars_matches_pandas(
    ...
    assert result_pd["ancestor_origin_time"].tolist() == (
