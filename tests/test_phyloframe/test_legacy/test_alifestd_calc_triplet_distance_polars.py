...
def test_polytomy_identical(df: pl.DataFrame, apply: typing.Callable):
    df = _prepare(df)
    assert alifestd_calc_triplet_distance_polars(apply(df), apply(df)) == 0
...
def test_differing_wrong1(apply: typing.Callable):
    ...
    assert (
        0 < alifestd_calc_triplet_distance_polars(apply(adf), apply(bdf)) < 1
    )


def test_differing_wrong2(apply: typing.Callable):
    adf = _prepare(
        pl.DataFrame(
            {
                "id": [0, 1, 2, 3, 4, 5],
...
def test_identical_polytomy1(apply: typing.Callable):
    adf = _prepare(
        pl.DataFrame(
            {
...
def test_differing_polytomy_asymmetrical_strict(apply: typing.Callable):
    adf = _prepare(
        pl.DataFrame(
            {
                "id": [0, 1, 2, 3, 4, 5],
