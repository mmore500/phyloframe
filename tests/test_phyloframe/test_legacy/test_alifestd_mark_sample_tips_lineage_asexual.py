...
def test_alifestd_mark_sample_tips_lineage_asexual(seed, n_downsample):
    ...
    assert result["alifestd_mark_sample_tips_lineage_asexual"].dtype == bool
    assert len(result) == len(original_df)
    pd.testing.assert_frame_equal(phylogeny_df, original_df)


def test_alifestd_mark_sample_tips_lineage_asexual_mark_as():
    phylogeny_df = alifestd_to_working_format(
        pd.read_csv(f"{assets_path}/nk_ecoeaselection.csv")
    )
...
def test_alifestd_mark_sample_tips_lineage_asexual_empty():
    phylogeny_df = pd.DataFrame(
        {"id": [], "parent_id": [], "ancestor_id": [], "origin_time": []}
    )

    result = alifestd_mark_sample_tips_lineage_asexual(phylogeny_df, 1)
