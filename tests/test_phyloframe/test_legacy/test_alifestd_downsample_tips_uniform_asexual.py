...
from phyloframe.legacy._alifestd_downsample_tips_uniform_asexual import (
    alifestd_downsample_tips_uniform_asexual as alifestd_downsample_tips_uniform_asexual_,
)

from ._impl import enforce_dtype_stability_pandas

alifestd_downsample_tips_uniform_asexual = enforce_dtype_stability_pandas(
    alifestd_downsample_tips_uniform_asexual_
)
...
def test_alifestd_downsample_tips_uniform_asexual(
    phylogeny_df, n_downsample, mutate, seed
):
    original_df = phylogeny_df.copy()

    result_df = alifestd_downsample_tips_uniform_asexual(
        phylogeny_df, n_downsample, mutate, seed
    )
...
def test_alifestd_downsample_tips_uniform_asexual_with_zero_tips(n_downsample):
...
@pytest.mark.parametrize("seed", [1, 42])
def test_prune_tips_vs_downsample_uniform(phylogeny_df, n_downsample, seed):
    original_df = phylogeny_df.copy()

    downsampled_df = alifestd_downsample_tips_uniform_asexual(
