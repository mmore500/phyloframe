...
def test_alifestd_mark_leaves_polars_cli_csv():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_leaves_polars.csv"  # nosec B108
    )
...
def test_alifestd_mark_leaves_polars_cli_parquet():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_leaves_polars.pqt"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
...
def test_alifestd_mark_leaves_polars_cli_mark_as():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_leaves_polars_mark_as.csv"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
...
def test_alifestd_mark_leaves_polars_cli_empty():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_leaves_polars_empty.csv"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
