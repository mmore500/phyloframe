...
def test_alifestd_topological_sort_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_topological_sort",
            "--help",
        ],
        check=True,
    )
...
def test_alifestd_topological_sort_cli_csv():
    output_file = "/tmp/phyloframe_alifestd_topological_sort.csv"  # nosec B108
...
def test_alifestd_topological_sort_cli_parquet():
    output_file = "/tmp/phyloframe_alifestd_topological_sort.pqt"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
