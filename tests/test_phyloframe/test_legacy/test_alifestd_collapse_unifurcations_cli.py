...
def test_alifestd_collapse_unifurcations_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_collapse_unifurcations",
            "--help",
        ],
        check=True,
    )
...
def test_alifestd_collapse_unifurcations_cli_csv():
    output_file = (
        "/tmp/phyloframe_alifestd_collapse_unifurcations.csv"  # nosec B108
    )
...
def test_alifestd_collapse_unifurcations_cli_parquet():
    output_file = (
        "/tmp/phyloframe_alifestd_collapse_unifurcations.pqt"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
...
def test_alifestd_collapse_unifurcations_cli_ignore_topological_sensitivity():
    output_file = "/tmp/phyloframe_alifestd_collapse_unifurcations_ignore.csv"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
...
def test_alifestd_collapse_unifurcations_cli_drop_topological_sensitivity():
    output_file = "/tmp/phyloframe_alifestd_collapse_unifurcations_drop.csv"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
