...
def test_alifestd_assign_contiguous_ids_polars_cli_csv():
    output_file = "/tmp/phyloframe_alifestd_assign_contiguous_ids_polars.csv"  # nosec B108
...
def test_alifestd_assign_contiguous_ids_polars_cli_parquet():
    output_file = "/tmp/phyloframe_alifestd_assign_contiguous_ids_polars.pqt"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
