...
def test_alifestd_mark_leaves_cli_csv():
    output_file = "/tmp/phyloframe_alifestd_mark_leaves.csv"  # nosec B108
...
def test_alifestd_mark_leaves_cli_parquet():
    output_file = "/tmp/phyloframe_alifestd_mark_leaves.pqt"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
...
def test_alifestd_mark_leaves_cli_mark_as():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_leaves_mark_as.csv"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
