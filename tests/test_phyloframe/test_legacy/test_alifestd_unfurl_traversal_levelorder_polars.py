...
def test_empty():
    ...
    result = alifestd_unfurl_traversal_levelorder_polars(df)
    assert len(result) == 0
...
def test_simple4():
    ...
    Depths: 0=0, 1=1, 2=1, 4=1, 3=2.
    Levelorder: 0, 1, 2, 4, 3
    """
    df = _make_contiguous_df(np.array([0, 0, 0, 1, 0]))
    result = alifestd_unfurl_traversal_levelorder_polars(df)
    assert result.tolist() == [0, 1, 2, 4, 3]


def test_multi_root():
    """Two roots: 0 -> {2}, 1 -> {3}.

    Depths: 0=0, 1=0, 2=1, 3=1.
    Levelorder: 0, 1, 2, 3
    """
...
def test_star():
    ...
    Depths: 0=0, 1=1, 2=1, 3=1, 4=1.
    Levelorder: 0, 1, 2, 3, 4
    """
...
def test_valid_levelorder():
    """Depths must be nondecreasing and every parent before its children."""
    ...
    # Check nondecreasing depths
    from phyloframe.legacy import (
        _alifestd_mark_node_depth_asexual as depth_mod,
    )

    depths = depth_mod._alifestd_calc_node_depth_asexual_contiguous(
        ancestor_ids,
    )
    result_depths = [depths[node] for node in result_list]
    assert all(a <= b for a, b in zip(result_depths, result_depths[1:]))
...
def test_fuzz(phylogeny_csv: str):
    ...
    result = alifestd_unfurl_traversal_levelorder_polars(phylogeny_df)
    ...
    # Verify valid levelorder: every parent before its children
    ...
    result_depths = [depths[node] for node in result]
    assert all(a <= b for a, b in zip(result_depths, result_depths[1:]))
