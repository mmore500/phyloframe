...
def test_alifestd_reroot_at_id_asexual_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_reroot_at_id_asexual",
            "--new-root-id",
            "0",
            "--help",
        ],
...
def test_alifestd_reroot_at_id_asexual_cli_csv():
    output_file = (
        "/tmp/phyloframe_alifestd_reroot_at_id_asexual.csv"  # nosec B108
    )
...
def test_alifestd_reroot_at_id_asexual_cli_parquet():
    output_file = (
        "/tmp/phyloframe_alifestd_reroot_at_id_asexual.pqt"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
