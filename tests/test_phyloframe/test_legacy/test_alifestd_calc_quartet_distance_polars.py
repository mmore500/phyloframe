...
def test_polytomy_identical(df: pl.DataFrame):
    for adf, bdf in (df, df), (df, df.sample(fraction=1.0, shuffle=True)):
        assert alifestd_calc_quartet_distance_polars(adf, bdf) == 0
...
def test_differing_wrong1():
    ...
    assert 0 < alifestd_calc_quartet_distance_polars(adf, bdf) < 1
    assert (
        0
        < alifestd_calc_quartet_distance_polars(
            adf,
...
def test_differing_wrong_big():
    ...
    est = alifestd_calc_quartet_distance_polars(adf, bdf, "taxon_label")
    assert 0 < est < 1
...
def test_identical_polytomy():
    ...
    assert alifestd_calc_quartet_distance_polars(adf, bdf, "taxon_label") == 0
    assert (
        alifestd_calc_quartet_distance_polars(
            adf,
            bdf.sample(fraction=1.0, shuffle=True),
...
def test_differing_polytomy_asymmetrical():
    ...
    assert (
        0 < alifestd_calc_quartet_distance_polars(adf, bdf, "taxon_label") < 1
    )
    assert (
        0
        < alifestd_calc_quartet_distance_polars(
