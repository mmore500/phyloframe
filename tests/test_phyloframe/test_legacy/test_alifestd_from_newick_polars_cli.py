...
def test_alifestd_from_newick_polars_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_from_newick_polars",
            "--help",
        ],
        check=True,
    )
...
def test_alifestd_from_newick_polars_cli_version():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
...
def test_alifestd_from_newick_polars_cli_csv(tmp_path: pathlib.Path):
    ...
    assert "Homo_sapiens" in set(result["taxon_label"].to_list())


def test_alifestd_from_newick_polars_cli_replace_unquoted(
