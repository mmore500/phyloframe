...
def test_alifestd_add_global_root_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_add_global_root",
            "--help",
        ],
        check=True,
    )
...
def test_alifestd_add_global_root_cli_csv():
    output_file = "/tmp/phyloframe_alifestd_add_global_root.csv"  # nosec B108
...
def test_alifestd_add_global_root_cli_parquet():
    output_file = "/tmp/phyloframe_alifestd_add_global_root.pqt"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
...
def test_alifestd_add_global_root_cli_ignore_topological_sensitivity():
    output_file = (
        "/tmp/phyloframe_alifestd_add_global_root_ignore.csv"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
...
def test_alifestd_add_global_root_cli_drop_topological_sensitivity():
    output_file = (
        "/tmp/phyloframe_alifestd_add_global_root_drop.csv"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
