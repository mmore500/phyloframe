...
def test_alifestd_coarsen_dilate_asexual_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_coarsen_dilate_asexual",
            "--dilation",
            "1",
            "--help",
        ],
...
def test_alifestd_coarsen_dilate_asexual_cli_csv():
    output_file = (
        "/tmp/phyloframe_alifestd_coarsen_dilate_asexual.csv"  # nosec B108
    )
...
def test_alifestd_coarsen_dilate_asexual_cli_parquet():
    output_file = (
        "/tmp/phyloframe_alifestd_coarsen_dilate_asexual.pqt"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
