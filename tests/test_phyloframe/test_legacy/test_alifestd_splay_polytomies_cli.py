...
def test_alifestd_splay_polytomies_cli_help():
    subprocess.run(  # nosec B603
        [
            "python3",
            "-m",
            "phyloframe.legacy._alifestd_splay_polytomies",
            "--help",
        ],
        check=True,
    )
...
def test_alifestd_splay_polytomies_cli_csv():
    output_file = "/tmp/phyloframe_alifestd_splay_polytomies.csv"  # nosec B108
...
def test_alifestd_splay_polytomies_cli_parquet():
    output_file = "/tmp/phyloframe_alifestd_splay_polytomies.pqt"  # nosec B108
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
            "python3",
