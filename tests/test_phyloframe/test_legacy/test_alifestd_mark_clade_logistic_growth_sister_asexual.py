...
alifestd_mark_clade_logistic_growth_sister_asexual = enforce_dtype_stability_pandas(
...
def test_empty():
    ...
    res = alifestd_mark_clade_logistic_growth_sister_asexual(
        phylogeny_df,
    )
    assert len(res) == 0
...
def test_simple2(mutate: bool, parallel_backend: typing.Optional[str]):
    ...
    original_df = phylogeny_df.copy()
    result_df = alifestd_mark_clade_logistic_growth_sister_asexual(
        phylogeny_df,
        mutate=mutate,
        parallel_backend=parallel_backend,
    )
...
def test_simple4(
    ...
    result_df.index = result_df["id"]
    assert math.isnan(result_df.loc[0, "clade_logistic_growth_sister"])
    assert result_df.loc[1, "clade_logistic_growth_sister"] < 0
    assert result_df.loc[2, "clade_logistic_growth_sister"] > 0
    assert result_df.loc[3, "clade_logistic_growth_sister"] > 0
    assert result_df.loc[4, "clade_logistic_growth_sister"] < 0
    assert result_df.loc[5, "clade_logistic_growth_sister"] < 0
    assert result_df.loc[6, "clade_logistic_growth_sister"] > 0
...
def test_simple7(mutate: bool, parallel_backend: typing.Optional[str]):
    ...
    result_df.index = result_df["id"]
    assert math.isnan(result_df.loc[3, "clade_logistic_growth_sister"])
    assert result_df.loc[0, "clade_logistic_growth_sister"] > 0
    assert math.isnan(result_df.loc[1, "clade_logistic_growth_sister"])
    assert result_df.loc[2, "clade_logistic_growth_sister"] < 0
    assert math.isnan(result_df.loc[4, "clade_logistic_growth_sister"])
...
def test_simple8(
    ...
    assert result_df.loc[3, "clade_logistic_growth_sister"] > 0
    assert math.isnan(result_df.loc[4, "clade_logistic_growth_sister"])
    assert math.isnan(result_df.loc[5, "clade_logistic_growth_sister"])
    assert math.isnan(result_df.loc[6, "clade_logistic_growth_sister"])
...
def test_fuzz(
    ...
    # Verify result_df
    assert "clade_logistic_growth_sister" in result_df.columns
    assert len(result_df) == len(phylogeny_df)

    assert (
        result_df["clade_logistic_growth_sister"].isna()
        == result_df["is_root"]
    ).all()
