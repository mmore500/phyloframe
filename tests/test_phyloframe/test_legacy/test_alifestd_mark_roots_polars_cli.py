...
def test_alifestd_mark_roots_polars_cli_csv():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_roots_polars.csv"  # nosec B108
    )
...
def test_alifestd_mark_roots_polars_cli_parquet():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_roots_polars.pqt"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
...
def test_alifestd_mark_roots_polars_cli_mark_as():
    output_file = (
        "/tmp/phyloframe_alifestd_mark_roots_polars_mark_as.csv"  # nosec B108
    )
    pathlib.Path(output_file).unlink(missing_ok=True)
    subprocess.run(  # nosec B603
        [
