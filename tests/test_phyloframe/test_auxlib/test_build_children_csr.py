...
def _children_of(node, child_start, children_flat):
    """Extract children list for a node from CSR arrays."""
    return children_flat[child_start[node] : child_start[node + 1]].tolist()


def test_single_root():
    ancestor_ids = np.array([0], dtype=np.int64)
    num_children = np.array([0], dtype=np.int64)
    child_start, children_flat = build_children_csr(ancestor_ids, num_children)

    assert child_start.tolist() == [0, 0]
    assert _children_of(0, child_start, children_flat) == []
...
def test_chain():
    ...
    assert _children_of(0, child_start, children_flat) == [1]
    assert _children_of(1, child_start, children_flat) == [2]
    assert _children_of(2, child_start, children_flat) == []
...
def test_branching():
    ...
    assert _children_of(0, child_start, children_flat) == [1, 2]
    assert _children_of(1, child_start, children_flat) == [3]
    assert _children_of(2, child_start, children_flat) == []
    assert _children_of(3, child_start, children_flat) == []
...
def test_star():
    ...
    assert _children_of(0, child_start, children_flat) == [1, 2, 3, 4]
    for i in range(1, 5):
        assert _children_of(i, child_start, children_flat) == []
...
def test_multi_root():
    ...
    assert _children_of(0, child_start, children_flat) == [2]
    assert _children_of(1, child_start, children_flat) == [3]
    assert _children_of(2, child_start, children_flat) == []
    assert _children_of(3, child_start, children_flat) == []
...
def test_children_sorted_ascending():
    ...
    children = _children_of(0, child_start, children_flat)
...
def test_deep_tree():
    ...
    for i in range(n - 1):
        assert _children_of(i, child_start, children_flat) == [i + 1]
    assert _children_of(n - 1, child_start, children_flat) == []
