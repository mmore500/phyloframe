...
def alifestd_mark_colless_like_index_var_polars(
    ...
    logging.info(
        "- alifestd_mark_colless_like_index_var_polars: "
        "computing colless_like_index_var...",
    )
    result = _colless_like_fast_path(ancestor_ids, 1)

    return phylogeny_df.with_columns(
        colless_like_index_var=result,
    )
...
if __name__ == "__main__":
    ...
    try:
        with log_context_duration(
            "phyloframe.legacy"
            "._alifestd_mark_colless_like_index_var_polars",
            logging.info,
        ):
            _run_dataframe_cli(
                base_parser=parser,
