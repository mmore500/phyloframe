...
def alifestd_mark_clade_duration_ratio_sister_polars(
    ...
    if phylogeny_df.lazy().limit(1).collect().is_empty():
        return phylogeny_df.with_columns(
            clade_duration_ratio_sister=pl.lit(0.0).cast(pl.Float64),
        )

    logging.info(
        "- alifestd_mark_clade_duration_ratio_sister_polars: checking contiguous ids...",
    )
    ...
    logging.info(
        "- alifestd_mark_clade_duration_ratio_sister_polars: checking topological sort...",
    )
    ...
    result = clade_durations / clade_durations[sister_ids]

    return phylogeny_df.with_columns(
        clade_duration_ratio_sister=result,
    )
...
if __name__ == "__main__":
    ...
    try:
        with log_context_duration(
            ...
            _run_dataframe_cli(
                base_parser=parser,
                output_dataframe_op=alifestd_mark_clade_duration_ratio_sister_polars,
            )
