...
def alifestd_mark_colless_like_index_sd_polars(
    ...
    sample standard deviation as dissimilarity.
    """
    ...
    if phylogeny_df.lazy().limit(1).collect().is_empty():
        return phylogeny_df.with_columns(
            colless_like_index_sd=pl.lit(0.0).cast(pl.Float64),
        )

    logging.info(
        "- alifestd_mark_colless_like_index_sd_polars: checking contiguous ids...",
    )
    if not alifestd_has_contiguous_ids_polars(phylogeny_df):
        phylogeny_df = alifestd_assign_contiguous_ids_polars(phylogeny_df)

    logging.info(
        "- alifestd_mark_colless_like_index_sd_polars: checking topological sort...",
    )
