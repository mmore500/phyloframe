...
def alifestd_count_unifurcating_roots_polars(
    ...
    return (
        phylogeny_df.lazy()
        .select(
            (pl.col("is_root") & (pl.col("num_children") == 1)).sum(),
        )
        .collect()
        .item()
    )
...
if __name__ == "__main__":
    ...
    with log_context_duration(
        "phyloframe.legacy.alifestd_count_unifurcating_roots_polars",
        logging.info,
    ):
        count = alifestd_count_unifurcating_roots_polars(phylogeny_df)
