...
def alifestd_count_leaf_nodes_polars(phylogeny_df: pl.DataFrame) -> int:
    ...
    return (
        phylogeny_df.lazy()
        .select((pl.col("num_children") == 0).sum())
        .collect()
        .item()
    )
...
Print number of leaf nodes in alife-standard phylogeny.
...
if __name__ == "__main__":
    ...
    with log_context_duration(
        "phyloframe.legacy.alifestd_count_leaf_nodes_polars", logging.info
    ):
        count = alifestd_count_leaf_nodes_polars(phylogeny_df)
