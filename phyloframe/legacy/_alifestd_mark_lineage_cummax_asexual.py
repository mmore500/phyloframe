...
@jit(nopython=True)
def _alifestd_mark_lineage_cummax_asexual_fast_path(
    ancestor_ids: np.ndarray,
    values: np.ndarray,
    reverse: bool,
) -> np.ndarray:
    """Implementation detail for `alifestd_mark_lineage_cummax_asexual`."""
    ...
    for k in range(n - 1, -1, -1) if reverse else range(n):
        ...
        result[target] = max(result[anc], result[k])
    return result


def alifestd_mark_lineage_cummax_asexual(
    ...
    *,
    ...
    if "ancestor_id" not in phylogeny_df.columns:
        raise NotImplementedError(
            "alifestd_mark_lineage_cummax_asexual only supports asexual "
            "phylogenies.",
        )
    ...
    phylogeny_df[mark_as] = _alifestd_mark_lineage_cummax_asexual_fast_path(
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser = _add_parser_base(
        parser=parser,
        dfcli_module="phyloframe.legacy._alifestd_mark_lineage_cummax_asexual",
