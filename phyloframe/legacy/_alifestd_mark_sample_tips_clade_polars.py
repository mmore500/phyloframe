...
def _alifestd_mark_sample_tips_clade_polars_impl(
    ...
    logging.info(
        "- alifestd_mark_sample_tips_clade_polars: marking leaves...",
    )
    ...
    logging.info(
        "- alifestd_mark_sample_tips_clade_polars: marking num_leaves...",
    )
    ...
    logging.info(
        "- alifestd_mark_sample_tips_clade_polars: setting mark column...",
    )
    is_leaf = (
    ...
    marked = is_descendant & is_leaf
    phylogeny_df = phylogeny_df.with_columns(
        pl.Series(name=mark_as, values=marked),
    ).drop("alifestd_mask_descendants_polars")
    del is_descendant, is_leaf, marked
    gc.collect()
    log_memory_usage(logging.info)
...
def alifestd_mark_sample_tips_clade_polars(
    ...
    mark_as : str, default "alifestd_mark_sample_tips_clade_polars"
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser = _add_parser_base(
        parser=parser,
        dfcli_module="phyloframe.legacy._alifestd_mark_sample_tips_clade_polars",
    ...
    parser.add_argument(
        "--mark-as",
        default="alifestd_mark_sample_tips_clade_polars",
        type=str,
        help="Column name for the boolean mark (default: alifestd_mark_sample_tips_clade_polars).",
    )
    add_bool_arg(
        parser,
...
if __name__ == "__main__":
    ...
    try:
        with log_context_duration(
            "phyloframe.legacy._alifestd_mark_sample_tips_clade_polars",
            logging.info,
        ):
            _run_dataframe_cli(
                base_parser=parser,
