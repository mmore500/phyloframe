...
def alifestd_mark_csr_offsets_polars(
    ...
    *,
    ...
    logging.info(
        "- alifestd_mark_csr_offsets_polars: extracting ancestor ids...",
    )
    ancestor_ids = (
        phylogeny_df.lazy()
        .select("ancestor_id")
    ...
    logging.info(
        "- alifestd_mark_csr_offsets_polars: " "tabulating CSR offsets...",
    )
    csr_offsets = _alifestd_mark_csr_offsets_asexual_fast_path(ancestor_ids)

    return phylogeny_df.with_columns(
        pl.Series(csr_offsets).alias(mark_as),
    )
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser = _add_parser_base(
        parser=parser,
        dfcli_module="phyloframe.legacy._alifestd_mark_csr_offsets_polars",
...
if __name__ == "__main__":
    ...
    try:
        with log_context_duration(
            "phyloframe.legacy._alifestd_mark_csr_offsets_polars",
            logging.info,
        ):
            _run_dataframe_cli(
                base_parser=parser,
                output_dataframe_op=functools.partial(
                    alifestd_mark_csr_offsets_polars, mark_as=args.mark_as
                ),
            )
