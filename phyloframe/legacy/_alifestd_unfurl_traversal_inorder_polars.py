...
from ._alifestd_unfurl_traversal_inorder_asexual import (
    _alifestd_unfurl_traversal_inorder_asexual_fast_path,
)


def alifestd_unfurl_traversal_inorder_polars(
    phylogeny_df: pl.DataFrame,
) -> np.ndarray:
    """List node indices in inorder traversal order, with left children
    visited first.
    ...
    if "right_child_id" not in schema_names:
        right_children = _alifestd_mark_right_child_asexual_fast_path(
            ancestor_ids,
        )
    else:
        right_children = (
