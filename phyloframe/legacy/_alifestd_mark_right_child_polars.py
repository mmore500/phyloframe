...
def alifestd_mark_right_child_polars(
    phylogeny_df: pl.DataFrame,
) -> pl.DataFrame:
    """Add column `right_child_id`, containing for each node its largest-id
    child.
    ...
    logging.info(
        "- alifestd_mark_right_child_polars: adding ancestor_id col...",
    )
    phylogeny_df = alifestd_try_add_ancestor_id_col_polars(phylogeny_df)

    if phylogeny_df.lazy().limit(1).collect().is_empty():
        return phylogeny_df.with_columns(
            right_child_id=pl.lit(0).cast(pl.Int64),
        )
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser = _add_parser_base(
        parser=parser,
        dfcli_module="phyloframe.legacy._alifestd_mark_right_child_polars",
...
if __name__ == "__main__":
    ...
    try:
        with log_context_duration(
            ...
            _run_dataframe_cli(
                base_parser=parser,
                output_dataframe_op=alifestd_mark_right_child_polars,
            )
