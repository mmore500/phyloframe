...
@jit(nopython=True)
def _alifestd_mark_prev_sibling_id_asexual_fast_path(
    ancestor_ids: np.ndarray,
) -> np.ndarray:
    """Implementation detail for `alifestd_mark_prev_sibling_id_asexual`.

    Returns array where each node's value is the next-lowest id with the same
    parent, or own id if no such sibling exists.
    """
    n = len(ancestor_ids)
    prev_sibling_ids = np.arange(n, dtype=ancestor_ids.dtype)  # default: self
    ...
    for idx, parent in enumerate(ancestor_ids):
        if parent != idx:  # skip genesis/root self-ref
            ...
            if prev != -1:
                prev_sibling_ids[idx] = prev
            last_child_with_parent[parent] = idx

    return prev_sibling_ids


def _alifestd_mark_prev_sibling_id_asexual_slow_path(
    phylogeny_df: pd.DataFrame,
) -> pd.DataFrame:
    """Implementation detail for `alifestd_mark_prev_sibling_id_asexual`."""
    phylogeny_df.index = phylogeny_df["id"]

    phylogeny_df["prev_sibling_id"] = phylogeny_df["id"]
    ...
    for idx in phylogeny_df.index:
        ...
        if ancestor_id in last_child_with_parent:
            ...
            phylogeny_df.at[idx, "prev_sibling_id"] = prev
...
def alifestd_mark_prev_sibling_id_asexual(
    ...
    *,
    ...
    if alifestd_has_contiguous_ids(phylogeny_df):
        phylogeny_df[
            mark_as
        ] = _alifestd_mark_prev_sibling_id_asexual_fast_path(
            phylogeny_df["ancestor_id"].to_numpy(),
        )
        return phylogeny_df
    else:
        return _alifestd_mark_prev_sibling_id_asexual_slow_path(
            phylogeny_df,
        )
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser = _add_parser_base(
        parser=parser,
        dfcli_module="phyloframe.legacy._alifestd_mark_prev_sibling_id_asexual",
...
if __name__ == "__main__":
    ...
    with log_context_duration(
        "phyloframe.legacy._alifestd_mark_prev_sibling_id_asexual",
        logging.info,
    ):
        _run_dataframe_cli(
            base_parser=parser,
            output_dataframe_op=delegate_polars_implementation()(
                functools.partial(
                    alifestd_mark_prev_sibling_id_asexual, mark_as=args.mark_as
                ),
            ),
        )
