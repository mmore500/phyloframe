...
def alifestd_mark_prev_sibling_id_polars(
    ...
    logging.info(
        "- alifestd_mark_prev_sibling_id_polars: checking contiguous ids...",
    )
    if not alifestd_has_contiguous_ids_polars(phylogeny_df):
        phylogeny_df = alifestd_assign_contiguous_ids_polars(phylogeny_df)

    logging.info(
        "- alifestd_mark_prev_sibling_id_polars: checking topological sort...",
    )
    ...
    logging.info(
        "- alifestd_mark_prev_sibling_id_polars: extracting ancestor ids...",
    )
    ancestor_ids = (
        phylogeny_df.lazy()
        .select("ancestor_id")
