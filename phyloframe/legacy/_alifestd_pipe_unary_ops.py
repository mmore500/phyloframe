...
def alifestd_pipe_unary_ops(
    phylogeny_df: pd.DataFrame,
    *unary_ops: typing.Callable[[pd.DataFrame], pd.DataFrame],
    ...
    *unary_ops : callable
    ...
    This function also accepts a polars.DataFrame, for which there is a
    separate delegated implementation.
    """
    for op in progress_wrap(unary_ops):
...
def _create_parser() -> argparse.ArgumentParser:
    ...
    parser = _add_parser_base(
        parser=parser,
        dfcli_module="phyloframe.legacy._alifestd_pipe_unary_ops",
...
if __name__ == "__main__":
    ...
    with log_context_duration(
        "phyloframe.legacy._alifestd_pipe_unary_ops", logging.info
    ):
        _run_dataframe_cli(
            base_parser=parser,
            output_dataframe_op=delegate_polars_implementation()(
                lambda df: alifestd_pipe_unary_ops(df, *unary_ops)
            ),
        )
