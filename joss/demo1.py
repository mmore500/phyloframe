...
def _(np, pfl, plot, tp):
    ...
    df_res = (
        ...
        .pipe(  # flip rerooted branch lengths
            lambda df: df.assign(
                branch_length=np.where(
                    df_raw.loc[df["id"], "ancestor_id"]
                    != df["ancestor_id"],  # where flipped
                    df_raw.loc[df["ancestor_id"], "branch_length"],  # ancestor's
                    df_raw.loc[df["id"], "branch_length"],  # ...otherwise own
                ),
            ),
        )
