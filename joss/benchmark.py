...
parse/make and file format save/load.  Any single operation exceeding
...
# Operations whose return value is a byte count (not a duration).
BYTE_OPS = {
    "memory_bytes",
    "newick_bytes",
    "newick_gzip_bytes",
...
def _run_in_child(conn, bench_cls, newick, op, return_value):
    ...
    try:
        ...
        if op not in ("parse_newick", "memory_bytes"):
            bench.parse_newick()
        # Run any op-specific setup (e.g. write temp files for load ops).
        setup_fn = getattr(bench, "_setup_op", None)
...
class PhyloframeBench:
    ...
    def __init__(self, newick):
        ...
        self._tmp_csv = None
        self._tmp_feather = None
        self._tmp_parquet = None
    ...
    def _get_export_df(self):
        """Get df with id dropped and dtypes shrunk for file benchmarks."""
        if self._export_df is None:
            import polars as pl

            df = self._ensure_df().drop("id")
            self._export_df = df.select(pl.all().shrink_dtype())
        return self._export_df

    def _write_temp_files(self):
        """Write export df to temp CSV, feather, and parquet files."""
        import tempfile
        ...
        self._tmp_feather = tempfile.NamedTemporaryFile(
            suffix=".feather", delete=False
        ).name
        self._tmp_parquet = tempfile.NamedTemporaryFile(
            suffix=".parquet", delete=False
        ).name
        df.write_csv(self._tmp_csv)
        df.write_ipc(self._tmp_feather)
        df.write_parquet(self._tmp_parquet)

    _FILE_FORMAT_OPS = frozenset(
        {
            "save_csv",
            "save_feather",
            "save_parquet",
    ...
    _LOAD_FILE_OPS = frozenset({"load_csv", "load_feather", "load_parquet"})

    def _setup_op(self, op):
        """Pre-compute export df or write temp files before timing."""
        if op in self._FILE_FORMAT_OPS:
            self._get_export_df()
        if op in self._LOAD_FILE_OPS:
    ...
    def load_csv(self):
        ...
        pl.read_csv(self._tmp_csv)

    def load_feather(self):
        import polars as pl

        pl.read_ipc(self._tmp_feather)

    def load_parquet(self):
        import polars as pl

        pl.read_parquet(self._tmp_parquet)
    ...
    def newick_bytes(self):
        return len(self._newick.encode("utf-8"))

    def newick_gzip_bytes(self):
        import gzip

        return len(gzip.compress(self._newick.encode("utf-8")))
    ...
    for n_leaves in sizes:
        ...
        for LibClass in LIBRARIES:
            ...
            for op in OPERATIONS:
                ...
                if status == "SUCCESS" and op in BYTE_OPS:
                    label = f"{value:,} B"
                elif status == "SUCCESS":
                    label = f"{value:.4f}s"
                else:
